...
description: Native Humungousaur skill for Stable Diffusion. Use when a task calls for stable diffusion workflows, readiness checks, artifacts, or approval-gated local/provider actions.
---
...
Use this skill when the user asks for stable diffusion planning, execution, verification, troubleshooting, or artifact creation inside Humungousaur.
...
- `tool_output_store`
- `stable_diffusion_generation_prepare`
- `execute_code`
- `python_interpreter`
- `process`
- `terminal`
...
1. Clarify the user's concrete stable diffusion objective, target environment, credentials already configured, and expected artifact or action.
