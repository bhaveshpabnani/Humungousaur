...
description: Native Humungousaur skill for Jupyter Live Kernel. Use when a task calls for jupyter live kernel workflows, readiness checks, artifacts, or approval-gated local/provider actions.
---
...
Use this skill when the user asks for jupyter live kernel planning, execution, verification, troubleshooting, or artifact creation inside Humungousaur.
...
- `tool_output_store`
- `jupyter_live_kernel_execute_prepare`
- `execute_code`
- `python_interpreter`
- `process`
- `terminal`
...
1. Clarify the user's concrete jupyter live kernel objective, target environment, credentials already configured, and expected artifact or action.
