...
description: Native Humungousaur skill for Kanban Orchestrator. Use when a task calls for kanban orchestrator workflows, readiness checks, artifacts, or approval-gated local/provider actions.
---
...
Use this skill when the user asks for kanban orchestrator planning, execution, verification, troubleshooting, or artifact creation inside Humungousaur.
...
- `tool_output_store`
- `kanban_orchestrator_plan_create`
- `delegate_task`
- `kanban_create`
- `kanban_list`
- `kanban_heartbeat`
...
1. Clarify the user's concrete kanban orchestrator objective, target environment, credentials already configured, and expected artifact or action.
