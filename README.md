...
The long-term human-like assistant architecture lives in `docs/COGNITIVE_AGENT_ARCHITECTURE.md`. The first cognitive runtime layers are implemented as durable event, goal/task, focus, persona, knowledge, learning, consolidation, curation, recovery, briefing, wakeup, skill, specialist, and reflection stores; planner-visible cognitive state tools; model-led attention, reflection, consolidation, curation, recovery, and briefing decisions with explicit fallback; and a bounded autonomous runtime loop with queued events, due wakeups, task graphs, explicit delegation, pause/interrupt boundaries, completion gates, evidence-backed learning, adaptive repair tasks, current-work briefings, model-led memory hygiene, model-led experience consolidation, idle stopping, and cycle summaries.
